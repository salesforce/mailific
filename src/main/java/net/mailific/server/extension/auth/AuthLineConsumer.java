  ...
  public Transition consume(SmtpSession session, Line line) {
    ...
    if (encodedResponse.length == 1 && encodedResponse[0] == '*') {
      try {
        ...
        session.getEventLogger().error("SASL_CANCEL_ERROR", e, e.getMessage());
      }
    ...
    try {
      ...
      session.getEventLogger().info("SASL_DECODE_ERROR", null, "Response was not valid base64");
    ...
    try {
      ...
      session.getEventLogger().info("SASL_EVAL_ERROR", e, e.getMessage());
